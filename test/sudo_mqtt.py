...
def push_data(date_time,pulse,pressupper,presslower,oxi,pulse2):
    ...
    x = {"device_token":"aaa1-aaa1","date":str(date_time[0]),"time":str(hh.strftime("%H")+":"+hh.strftime("%M")),"BloodPress": [{"SYS": str(pressupper),"DIA": str(presslower),"PUL": str(pulse),}],"Oximeter": [{"SAT": str(oxi),"PUL": str(pulse2),}],}
    y = json.dumps(x)
